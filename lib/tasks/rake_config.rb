  ...
  class << self
    ...
    def config(is_cc_local_worker=false)
      ...
      if is_cc_local_worker
        local_worker_config= VCAP::CloudController::Config.read_file(cc_local_worker_config_file)
        return VCAP::CloudController::Config.load_from_hash(api_config.config_hash.deep_merge(local_worker_config))
      end
