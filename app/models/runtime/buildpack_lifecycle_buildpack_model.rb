...
module VCAP::CloudController
  class BuildpackLifecycleBuildpackModel < Sequel::Model(:buildpack_lifecycle_buildpacks)
    ...
    def validate
      ...
      elsif !UriUtils.is_buildpack_uri?(buildpack_url) && associations[:buildpack_lifecycle_data_model].present?
        errors.add(:buildpack_url, Sequel.lit("Specified invalid buildpack URL: \"#{buildpack_url}\""))
      elsif !UriUtils.is_cnb_buildpack_uri?(buildpack_url) && associations[:cnb_lifecycle_data_model].present?
        errors.add(:buildpack_url, Sequel.lit("Specified invalid buildpack URL: \"#{buildpack_url}\""))
      end
    end
  end
